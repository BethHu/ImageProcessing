...
BEGIN_MESSAGE_MAP(CFourierSpectrumDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CHECK_POWER, &CFourierSpectrumDlg::OnBnClickedCheckPower)
...
BOOL CFourierSpectrumDlg::OnInitDialog()
{
	...
	CheckDlgButton(IDC_CHECK_POWER, FALSE);
...
void CFourierSpectrumDlg::UpdateSpectrumDisplay()
{
	...
	// 检查2：谱类型选择
	if (!m_bShowAmplitude && !m_bShowPower && !m_bShowPhase && !m_bShowAngular && !m_bShowRadial)
	{
	...
	if (m_bShowPower)
	{
		...
		if (!spectrumImage.create(m_dftWidth, m_dftHeight, 1, 0.0))
		{
			AfxMessageBox(_T("无法创建能量谱图像"));
			TRACE(_T("能量谱图像创建失败\n"));
		}
		else
		{
			// 与幅度谱一致：由 m_bCenter 决定使用中心化/非中心化的 DFT 结果
			std::vector<std::complex<double>>* pDFTForDisplay =
				m_bCenter ? &m_dftResult : &m_dftResultCentered;

			TRACE(_T("使用%s的DFT结果进行能量谱显示\n"),
				m_bCenter ? _T("非中心化") : _T("中心化"));
			...
			for (int i = 0; i < totalPixels; i++)
			{
				spectrumImage.m_data[i] = std::norm((*pDFTForDisplay)[i]);
			}

			TRACE(_T("原始能量计算完成，总像素数：%d\n"), totalPixels);

			// 2) 应用对数变换（与幅度谱共用同一套参数）
			if (m_bLogTransform)
			{
				m_dLogBase = _ttof(m_strLogBase);
				m_dLogOffset = _ttof(m_strLogOffset);
				...
				TRACE(_T("能量谱：应用对数变换完成，base=%f, offset=%f\n"), m_dLogBase, m_dLogOffset);
			}

			// 3) 应用归一化/显示范围（与幅度谱共用同一套参数）
			if (m_bNormalizeAmp)
			{
				m_dDisplayMin = _ttof(m_strDisplayMin);
				m_dDisplayMax = _ttof(m_strDisplayMax);
				...
				TRACE(_T("能量谱归一化前：min=%f, max=%f\n"), currentMin, currentMax);
				TRACE(_T("能量谱目标范围：[%f, %f]\n"), m_dDisplayMin, m_dDisplayMax);
			...
			CString title;
			title.Format(_T("能量谱 (%s)"), m_bCenter ? _T("非中心化") : _T("中心化"));
			CImageDisplay::show(spectrumImage, this, title, 1, 1, 1, 0);
			hasDisplayedAny = TRUE;
			TRACE(_T("能量谱显示成功\n"));
		}
...
{
	// 幅度/能量谱参数（两者参数联动复用）
	BOOL enableAmpParams = (m_bShowAmplitude || m_bShowPower);
	...
	BOOL enableApplyButton = m_bShowAmplitude || m_bShowPower || m_bShowPhase || 
...
void CFourierSpectrumDlg::OnBnClickedCheckPower()
{
	m_bShowPower = IsDlgButtonChecked(IDC_CHECK_POWER);
	UpdateParameterControls();
}
